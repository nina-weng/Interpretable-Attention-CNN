...
# 'Segmentation_task' (dataset: 'dots')
# 'Advanced_Direction_task' (dataset: 'dots')
...
def build_file_name():
    ...
    all_EEG_file = all_EEG_file + '_min_prep_synch'
    all_EEG_file = all_EEG_file + ('_hilbert.npz' if config['feature_extraction'] else '_top5000.npz')#'.npz')
    if config['task'] == 'Advanced_Direction_task':
        all_EEG_file = 'Direction_task_with_dots_minprep_synch_500_nina_final_top5000.npz'
    return all_EEG_file
...
def create_folder():
    ...
    parser.add_argument("-d","--jobD", help="Job description",type=str,default='')
    args = parser.parse_args()
    if config['retrain']:
        model_folder_name = str(int(time.time()))
        model_folder_name += '_' + config['task'] + '_' + config['dataset'] + '_' + config['preprocessing'] + '_' + args.jobD
